class test:
	...
	def __init__(self):
		...
		self.__rgba = {'white':(1.0,1.0,1.0,1.0), 
		...
		def restype(self, residue):
			return __resdict[residue]

		def color(self, atom):
			try:
				return self.__colors[atom]
			except:
				return 'pink'

		def crgba(self, color):
			return self.__rgba[color]

		def colorrgba(self, atom):
			try:
				return self.__rgba[self.__colors[atom]]
			except:
				return self.__rgba['pink']

		def vrad(self, atom):
			try:
				return self.__radius[atom]
			except:
				return 1.5